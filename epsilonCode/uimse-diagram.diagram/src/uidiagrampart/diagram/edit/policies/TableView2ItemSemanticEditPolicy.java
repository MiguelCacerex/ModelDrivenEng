 ...
 * @generated
 */
public class TableView2ItemSemanticEditPolicy extends UidiagrampartBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public TableView2ItemSemanticEditPolicy() {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (UidiagrampartVisualIDRegistry.getVisualID(node)) {
			case TableViewTableViewLstColumnsCompartment2EditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (UidiagrampartVisualIDRegistry.getVisualID(cnode)) {
